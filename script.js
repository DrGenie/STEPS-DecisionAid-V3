/* ===================================================
   ...
   WTP based benefits, sensitivityy, Copilot integration and exports
   =================================================== */
...
function getConfigFromForm() {
  ...
  let scenarioName = scenarioNameEl ? (scenarioNameEl.value || "").trim() : "";
  if (!scenarioName) {
    ...
    // Reflect auto-name in the UI for transparency (user can still override).
...
function refreshTopScenariosPanel() {
  const body = document.getElementById("top5-table-body");
  const panel = document.getElementById("top5-panel");
  ...
  const items = (appState.savedScenarios || [])
    .map((s) => {
      const cap = s.capacity || computeCapacity(s.config);
      const feasible = cap.status === "Within current capacity";
      return { s, cap, feasible, score: computeScenarioRankScore(s, metric) };
    })
    .filter((x) => (feasibleOnly ? x.feasible : true))
    .sort((a, b) => (b.score || 0) - (a.score || 0))
    .slice(0, 5);

  const emptyNote = document.getElementById("top5-empty");
  if (items.length === 0) {
    body.innerHTML = "";
  ...
  items.forEach((item, idx) => {
    ...
    tr.innerHTML = `
      ...
      <td>${safeText(c.name || "Scenario")}</td>
      <td class="numeric-cell">${formatNumber(s.endorseRate, 1)}%</td>
      <td class="numeric-cell">${formatCurrencyDisplay(s.wtpAllCohorts, 0)}</td>
      <td class="numeric-cell">${formatCurrencyDisplay(s.natTotalCost, 0)}</td>
      <td class="numeric-cell">${formatCurrencyDisplay(s.epiBenefitAllCohorts, 0)}</td>
      <td class="numeric-cell">${formatCurrencyDisplay(s.netBenefitAllCohorts, 0)}</td>
      <td class="numeric-cell">${s.natBcr !== null ? formatNumber(s.natBcr, 2) : "-"}</td>
      <td><span class="pill ${cap.status === "Within current capacity" ? "pill-ok" : "pill-warn"}">${safeText(cap.status)}</span></td>
      <td class="numeric-cell">${formatNumber(cap.mentorShortfall || 0, 0)}</td>
      <td>
        <button type="button" class="mini" data-top5-action="snapshot" data-sid="${s.id}">View</button>
        <button type="button" class="mini" data-top5-action="toggle-shortlist" data-sid="${s.id}">${s.shortlisted ? "Unshortlist" : "Shortlist"}</button>
      </td>
    `;
    body.appendChild(tr);
  });

  // Wire up actions (safe to rebind; small table)
  body.querySelectorAll('button[data-top5-action]').forEach((btn) => {
    btn.addEventListener("click", (e) => {
      const action = e.currentTarget.getAttribute("data-top5-action");
      const id = e.currentTarget.getAttribute("data-sid");
      const sc = (appState.savedScenarios || []).find((x) => x.id === id);
      if (!sc) return;

      if (action === "snapshot") {
        openSnapshotModal(sc);
        return;
      }
      if (action === "toggle-shortlist") {
        sc.shortlisted = !sc.shortlisted;
...
function initEventHandlers() {
  ...
  if (top5RankBy) top5RankBy.addEventListener("change", () => refreshTopScenariosPanel());

  const top5FeasibleOnly = document.getElementById("top5-feasible-only");
  if (top5FeasibleOnly) top5FeasibleOnly.addEventListener("change", () => refreshTopScenariosPanel());

  const autoNameBtn = document.getElementById("auto-scenario-name");
  if (autoNameBtn) {
    autoNameBtn.addEventListener("click", () => {
      const c = getConfigFromForm();
      const name = buildDefaultScenarioName({
        tier: c.tier,
        career: c.career,
        mentorship: c.mentorship,
        delivery: c.delivery,
      ...
      if (el) el.value = name;
      showToast("Scenario name updated from the current configuration.", "success");
    });
  }
